...
export default function Walks() {
  ...
  return (
    // SLIDING CARD: Deep Charcoal (#0F161E)
    <section className="relative z-10 bg-[#0F161E] rounded-t-[60px] md:rounded-t-[100px] shadow-[0_-50px_100px_rgba(0,0,0,0.6)] pt-32 pb-24 border-t border-[#C19D60]/20 min-h-screen">
      
      <div className="max-w-7xl mx-auto px-6">
        
        {/* HEADER */}
        <div className="text-center mb-16">
          <span className="text-[#C19D60] font-sans font-bold tracking-[0.25em] uppercase text-xs">
            JOIN THE EXPEDITION
          </span>
          <h2 className="text-4xl md:text-6xl font-serif font-medium mt-4 text-[#F4F1EA] drop-shadow-lg">
            Upcoming Heritage Trails
          </h2>
          <p className="text-[#F4F1EA]/60 mt-4 max-w-2xl mx-auto font-light">
             Select a trail below to uncover the stories of the past.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {walks.map((walk) => {
            ...
            return (
              // CARD: Warm Sandstone (#E8E4D9)
              // This contrasts beautifully with the Dark Charcoal background.
              <div key={walk._id} className="group bg-[#E8E4D9] rounded-[2rem] overflow-hidden shadow-xl hover:-translate-y-3 transition-all duration-500 flex flex-col h-full border border-white/10">
                ...
                <div className="h-72 overflow-hidden relative cursor-pointer" onClick={() => setSelectedWalk(walk)}>
                  {walk.mainImage ? (
                    ...
                    <div className="w-full h-full bg-[#D6D3CD] flex items-center justify-center text-[#0F161E]/50">No Image</div>
                  )}
                  {/* Date Badge: Charcoal & Bronze */}
                  <div className="absolute top-4 left-4 bg-[#0F161E] text-[#F4F1EA] rounded-xl shadow-lg overflow-hidden flex flex-col items-center w-16 py-3 text-center border border-[#C19D60]/30">
                    <span className="text-[#C19D60] text-[10px] font-bold uppercase w-full tracking-widest">{month}</span>
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <div className="flex items-center gap-2 text-xs font-bold text-[#6D5D4B] uppercase tracking-widest mb-3">
                     <span className="w-2 h-2 rounded-full bg-[#C19D60]"></span>
                     {walk.location || 'Tamil Nadu'}
                  </div>

                  <h3 className="text-3xl font-serif font-medium mb-4 text-[#0F161E] group-hover:text-[#8D5524] transition-colors leading-tight cursor-pointer" onClick={() => setSelectedWalk(walk)}>
                    {walk.title}
                  </h3>
                  
                  <p className="text-[#4A4A4A] text-sm leading-relaxed mb-8 line-clamp-2 font-sans flex-grow">
                    {walk.description}
                  </p>
                  
                  <div className="mt-auto flex items-center justify-between pt-6 border-t border-[#0F161E]/10">
                    <div>
                      <span className="text-[#6D5D4B] text-xs uppercase tracking-wider block">Price</span>
                      <span className="text-xl font-bold text-[#0F161E]">₹{walk.price}</span>
                    </div>
                    {/* Arrow Button */}
                    <button 
                      onClick={() => setSelectedWalk(walk)}
                      className="w-12 h-12 rounded-full bg-[#0F161E] text-[#C19D60] flex items-center justify-center hover:bg-[#8D5524] hover:text-white transition-all duration-300"
                    >
      ...
      {selectedWalk && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#0F161E]/95 backdrop-blur-md transition-opacity" onClick={closeModal}></div>
          <div className="relative bg-[#F4F1EA] w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[90vh] animate-fadeInUp">
            ...
            <div className="w-full md:w-1/2 p-10 md:p-14 overflow-y-auto bg-[#F4F1EA]">
                {!showQR ? (
                  <>
                    <h2 className="text-4xl font-serif font-medium text-[#0F161E] mb-6">{selectedWalk.title}</h2>
                    <p className="text-[#4A4A4A] mb-8 leading-relaxed font-light text-lg">{selectedWalk.description}</p>
                    <button onClick={() => setShowQR(true)} className="w-full py-4 bg-[#0F161E] text-[#C19D60] font-bold uppercase tracking-widest hover:bg-[#8D5524] hover:text-white transition-colors rounded-xl shadow-lg">Confirm Booking</button>
                  </>
                ) : (
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    <button onClick={() => setShowQR(false)} className="self-start mb-4 text-xs font-bold uppercase text-gray-500">← Back</button>
                    <h3 className="text-2xl font-serif font-bold mb-4 text-[#0F161E]">Scan to Pay</h3>
                    {selectedWalk.qrCode && <img src={urlFor(selectedWalk.qrCode).width(300).url()} className="w-48 h-48 object-contain mb-4 border border-gray-200 rounded-lg p-2 bg-white" />}
                  </div>
                )}
            </div>
          </div>
